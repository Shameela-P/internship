				...
				<div>
						...
						<div class="min-w-0 grow pr-2">
							<div class="flex flex-wrap items-center gap-1.5">
								...
								{:else if intern.mode === 'Offline'}
									<span class="px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider bg-slate-100 border border-slate-200 text-slate-600">
										OFFLINE
									</span>
								{:else}
					...
					<div class="flex flex-wrap gap-1.5 mt-4">
						{#each intern.skillsRequired as skill}
							<span class="px-2 py-0.5 rounded bg-slate-100 border border-slate-200 text-[9px] font-semibold text-slate-600">
								{skill}
							</span>
						{/each}
					</div>
