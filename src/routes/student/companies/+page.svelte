				...
				<div class="group p-5 rounded-2xl bg-surface hover:bg-slate-50 border border-divider hover:border-blue-500/40 flex flex-col justify-between shadow-md hover:shadow-[0_0_20px_rgba(59,130,246,0.15)] hover:-translate-y-1 transition-all duration-300 min-h-[175px]">
					...
					<div class="mt-3 pt-3 border-t border-divider flex items-center justify-between">
						...
						<div class="flex items-center gap-3">
							...
							<a
								href={`/student/internships?companyId=${comp.id}&query=${encodeURIComponent(comp.companyName)}`}
		...
		>
			...
			<div class="space-y-4 text-xs text-slate-350">
				...
				<div class="flex items-center justify-between pt-4 border-t border-divider">
					...
					<a href={`/student/internships?companyId=${selectedCompanyDetail.id}&query=${encodeURIComponent(selectedCompanyDetail.companyName)}`} class="px-4 py-2 rounded-lg border border-divider hover:bg-slate-100 font-bold text-[10px] transition cursor-pointer text-slate-500 hover:text-primary">
						Explore Openings
					</a>
				</div>
			</div>
